...
{#snippet renderEntry(entry: Entry)}
      ...
      <div class="min-w-0 flex-1">
        ...
        {#if entry.duration}
          <p class="mt-1 text-sm text-gray-500">
            {formatDateRange(entry.duration, 'short')}
          </p>
        {/if}
      </div>
      {#if entry.logo}
  ...
  <section id="contact" class="space-y-12">
    <article id="me" class="border-b border-gray-200 pb-8">
      ...
      <div class="flex flex-wrap gap-6">
        {#each contact.socials as { social, url }}
          <a
            href={url}
      ...
      {#each skills as { icon, level }}
        <div class="flex flex-col items-center gap-1">
          <Icon
            ...
            class="h-8 w-8"
          />
          <span class="text-center text-sm">{icon.text}</span>
        </div>
      {/each}
    </div>
  </section>
