<script lang="ts">
    ...
    let { content, layout = 'left', padding = 0, children }: Props<any> = $props();
...
<section bind:this={sectionRef} class="w-screen snap-y snap-mandatory overflow-y-scroll h-screen">
    {#each content as { picture, link, id, data }, i}
        {@const shouldReverse = layout === 'right' || (layout === 'alternate' && i % 2 !== 0)}
        <article id={id} class="flex h-screen snap-center snap-always {shouldReverse ? 'flex-row-reverse' : ''}">
            <div class="w-1/2 flex items-center justify-center" style="padding: {padding}%; box-sizing: border-box;">
                <svelte:element this={link ? 'a' : 'div'} href={link} class="size-full overflow-hidden parallax-image">
                    <enhanced:img 
