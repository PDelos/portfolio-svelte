  ...
  $effect(() => {
    ...
    const handle = (e: WheelEvent) => {
      ...
      while (target && target !== document.documentElement) {
        ...
        const canScroll = window.getComputedStyle(target).overflowY !== 'hidden';
...
<div 
  class="fixed left-0 h-2 bg-blue-500 transition-all z-50 {position === 'bottom' ? 'bottom-0' : 'top-0'}"
